...
export default class Item extends ShopOptions {
    constructor(){
        super()
    ...
    generatePage(){
         ...
         const generateCards = ({
            ...
            function enumerationImg(img){
                let block = '';
                const count = img.length;
                for(let i = 0; i < count; i++){
                    block += `<div class="good-image__item">
                                <img src="${img[i]}" alt="${name} - ${description}">
                            </div>`
                }
                return block;
            }
            const imgBlock = enumerationImg(img);
                    ...
                    <div class="container">
                        <div class="breadcrumb">
                            <nav role="navigation" aria-label="Breadcrumb" class="breadcrumb__nav">
                                <ul class="breadcrumb__list">
                                    ...
                                    <li class="breadcrumb__list-item">
                                        <a href="#" class="breadcrumb__link"><span>${name}</span></a>
                                    </li>
                        ...
                        <div class="good wrapper">
                            ...
                            <div class="good-item"> <!-- not-in-stock - нет в наличии-->
                                ${count > this.counterNewProduct ? `<p class="good-item__new">Новинка</p>` : ''}
                                ...
                                <div class="good-item__section">
                                    ...
                                    <p class="good-item__price">
                                        ${!count ? `<span class="good-item__empty" style="display: block;">Нет в наличии</span>
                                ...
                                <div class="good-item__buttons">
                                    <button type="button" class="btn btn-good" data-idd="${id}">
                                        ...
                                        <span class="btn-good__label">${!count ? `Сообщить о поступлении` : `Добавить в корзину`}</span>
                                    </button>
                                    <button 
                                        type="button" 
                                        class = "btn btn-add-wishlist ${this.wishList.includes(id) ? 'contains-wishlist' : ''}"
                                        style=""
                                        data-idd="${id}">
            ...
            addInWishList(document.querySelector('.btn-add-wishlist'), this.wishList, name, id, this.setInLocalStorage);
            addInCart(document.querySelector('.btn-good'), this.cartList, name, id, this.setInLocalStorage);
         }
        ...
        if (location.hash && location.pathname.includes('card')) {
            ...
            this.data.item(hash, generateCards);
        }
    }
}